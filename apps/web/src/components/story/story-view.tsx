      ...
      <div className="min-w-0 space-y-5">
        ...
        <header className="space-y-1">
          ...
          <p className="text-[14px]">
            ...
            {story.embryo.contract ? (
              <>
                {' '}
                · contract <span className="code">{story.embryo.contract.id}</span> (
                {label(story.embryo.contract.status)})
              </>
            ) : null}
          </p>
            ...
            >
              <span>
                ...
                {story.recip.departure.rule.code ? (
                  <span className="code ml-1.5 text-[10px] text-muted-foreground">
                    {story.recip.departure.rule.code}
                  </span>
                ) : null}
              </span>
        ...
        <div className="grid gap-2 sm:grid-cols-4">
          ...
          <Stat
            ...
            sub={
              story.pregnancy.lastCheck ? `${story.pregnancy.lastCheck.id} · ${day(story.pregnancy.lastCheck.on)}` : ''
            }
            tone={
              ...
              <li key={`${row.kind}-${row.id}-${row.at}`}>
                ...
                >
                  ...
                  <span className="min-w-0">
                    ...
                    {row.detail ? <span className="block text-[12px] text-muted-foreground">{row.detail}</span> : null}
      ...
      <Sheet open={selected !== null} onOpenChange={(o) => (o ? undefined : setSelected(null))}>
        <SheetContent side="right" className="flex w-full flex-col gap-0 p-0 sm:max-w-[560px]">
          <SheetHeader className="border-b px-4 py-3">
            <SheetTitle className="text-[14px]">{selected?.kind === 'exception' ? 'Exception' : 'Evidence'}</SheetTitle>
