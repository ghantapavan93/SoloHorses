...
import { AlertTriangle, FileCheck2, HelpCircle, Landmark, RefreshCw, Repeat, ShieldAlert, Undo2 } from 'lucide-react';
...
import { StepRail, type Step } from '@/components/platform/step-rail';
...
export function SettlementView({
  ...
  const steps: Step[] = [
    ...
    },
    {
      ...
      note:
        documentStatus === 'HELD'
          ? `${scene.document?.id} · held by the rule`
          : documentStatus
            ? 'no longer held'
            : 'no document',
    },
  ...
  return (
    <div className="space-y-6">
      ...
      {/* ── the sequence ── */}
      <section className="rounded-md border" data-testid="settlement-steps">
        ...
        <div className="px-3 pb-3 pt-3 md:px-4 md:pb-4">
          <StepRail steps={steps} label="The sale's own order, from the hammer to the papers" />
        </div>
        {/* the controls, in context */}
        <div className="flex flex-wrap items-center gap-2 border-t bg-muted/30 px-3 py-2 text-[12px]">
          {scene.funds === 'PROCESSING' ? (
            <Button size="sm" className="h-7 text-[12px]" disabled={pending} onClick={settle} data-testid="settle-ach">
            ...
            <div className="flex items-start gap-2 rounded-md border border-critical/40 bg-critical/5 px-3 py-2 text-[13px]">
              ...
              <div>
                ...
                <p className="text-[12px] text-muted-foreground">
                  Nothing here picks a winner. Ask abstains — billing review is required — and the papers stay where
                  they are until a person decides on the board.
                </p>
              </div>
            </div>
          ) : null}
